...
```text
NAME
    sinject - secrets injection utility

SYNOPSIS
    sinject [OPTIONS] [FILE(s)...]
...
DESCRIPTION
    ...
    isnject will accept a list of files in which to inject secrets.  Within the files tokens can be defined which will be replaced by their corresponding secrets.
...
OPTIONS
    ...
    --errormode=none|secret|token|all
      ...
      none - sinject will never output pre-scanning errors
...
EXAMPLES
    sinject /opt/my_app/config/config.yml

    sinject --path="/mnt/secrets" /opt/my_app/config/config.yml

    sinject --secrets="database_user,database_password,api_token" /opt/my_app/config/config.yml

    sinject /opt/my_app/config/config.yml /opt/my_app/config/options.xml

    ls /opt/my_app/config_files/ | sinject
```
